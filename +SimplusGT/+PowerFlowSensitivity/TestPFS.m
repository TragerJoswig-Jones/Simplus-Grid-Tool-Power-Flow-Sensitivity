...
%ListLineTest = ListLineNew;   %  Updated with perturbations to load impedances to maintain constant loads
%ListLineTest = ListLine;  % Uses the old load impedance values assuming a fixed load impedance as is done in the simulation
...
dQ = -dQG + dQL;  % Note that power flow is stored in load convention
...
LOAD_SHIFTS = dPL.';
GEN_SHIFTS = -dPg.';    % Must use dPg (Instead of dPG) for indirect paramter changes (V at PQ bus)
QLOAD_SHIFTS = dQL.';   
QGEN_SHIFTS = -dQg.';   % Note generation is negated as power injections are stored in load convention
V_SHIFTS = dV.';
Th_SHIFTS = dTh.';
...
[PowerFlowNew] = SimplusGT.PowerFlow.PowerFlowNR_Hotstart(ListBusShift,ListLine,V1,del1,Wbase);
[ListBusNewPerturbed,ListLineNewPerturbed,PowerFlowNewLoad] = SimplusGT.PowerFlow.Load2SelfBranch(ListBusShift,ListLine,PowerFlowNew);

ListPowerFlowNew = SimplusGT.PowerFlow.Rearrange(PowerFlowNewLoad);
...
PowerFlowShifted = PowerFlowNewLoad;
...
for i = 1:NumApparatus
    if length(ApparatusBus{i}) == 1
 	    ApparatusPowerFlow{i} = PowerFlowShifted{ApparatusBus{i}};
    elseif length(ApparatusBus{i}) == 2
        ApparatusPowerFlow{i} = [PowerFlowShifted{ApparatusBus{i}(1)},PowerFlowShifted{ApparatusBus{i}(2)}];
    else
        error(['Error']);
    end
...
% ### Get the model of the network admittance
% [YbusObj,YbusDSS,~] = SimplusGT.Toolbox.YbusCalcDss(ListBus,ListLineNewPerturbed,Wbase);
% [~,lsw] = size(YbusDSS.B);
% ZbusObj = SimplusGT.ObjSwitchInOut(YbusObj,lsw);

% NOTE: Here we are using the old ZbusObj network admittance matrix.
% This assumes that the equivalent load impedances calculated based on 
% the voltage at the bus are fixed based on the initial power flow values, 
% and do not update reflecting the fixed power injection (assumed in the 
% power flow analysis) in our small-signal stability analysis.
%
% Practically, this would represent a load that behaves as a constant
% impedance over short-timescales (dynamic small-signal analysis) and as a 
% constant power load over longer-timescale (steady-state power flow analysis).
%
% In the dynamical simulations generated by Simplus-GT, the constant power 
% loads are represented as equivalent (in steady-statee) impedances that 
% remain constant when the system is perturbed, which aligns with the assumption
% made here.
